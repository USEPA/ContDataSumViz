      ...
      observeEvent(input$run_raster, {
        ...
        if (!all(is.na(data_to_plot[,mean_col]))){
          ...
          plot_output <- reactive({
            ggplot(data_to_plot, aes(x=as.Date(yday(Date),"2000-01-01"),y=year))+
          ...
          #   filename = function() {
          #     paste('raster', Sys.Date(), '.png', sep='')
          #   },
          #   content = function(file) {
          #     ggsave(file, plot_output(), width = 550 * (1/isolate(input$raster_plot_aspect_ratio)), height = 550, unit = "px")
          #   }
          # )
