...
#
#' @export
fun.DailyGageData <- function(fun.myData.SiteID
  ...
  tryCatch({
    while (intCounter < intCounter.Stop) {##WHILE.START
      ...
      #
      # Get available data
      data.what.dv <- dataRetrieval::whatNWISdata(siteNumber=strGage,service="dv")
      
      data.what.Codes <- data.what.dv[,"parm_cd"]
      ...
      myCode <- data.what.Codes[data.what.Codes %in% c("00060", "00065")] #"00060" #c("00060","00065")
      # can download multiple at one time

      data.myGage <- dataRetrieval::readNWISdv(strGage
                                               , myCode
